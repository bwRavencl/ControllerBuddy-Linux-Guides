...
2. Select **Proton Experimental** as compatibility tool.

3. Launch **Korea. IL-2 Series** so that the Proton prefix gets created.
   The game will likely crash on the first launch, but that's expected.

4. Export the `APP_ID` environment variable:
    ```sh
    export APP_ID=247970
    ```
...
10. Configure Korea. IL-2 Series to work with the [ControllerBuddy-Profiles](https://github.com/bwRavencl/ControllerBuddy-Profiles):
    ```sh
    ...
    cd "$controller_buddy_profiles_dir/configs/IL-2_Korea" &&
...
11. Update the **Korea. IL-2 Series** Steam shortcut as follows:
